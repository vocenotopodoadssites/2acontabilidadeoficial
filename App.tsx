...
import { 
  ...
  Calculator, TrendingUp, Coins, Building2, Percent
} from 'lucide-react';
  ...
  return (
    <div id="hero" ref={ref} className="bg-white min-h-screen flex flex-col justify-center relative overflow-hidden pt-20">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
           <img 
              ...
              alt="Background Contabilidade" 
              className="absolute inset-0 w-full h-full object-cover opacity-50"
           />
           {/* Overlay to ensure text readability */}
           <div className="absolute inset-0 bg-gradient-to-b from-white/95 via-white/80 to-white/50"></div>
        </div>
        ...
        {/* Original Floating Icons (Retained for depth) */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-[20%] left-[5%] text-[#0232ba]/5 animate-float-slow"><Calculator size={64} /></div>
          <div className="absolute bottom-[20%] right-[5%] text-[#0232ba]/5 animate-float-fast"><TrendingUp size={96} /></div>
        </div>
      ...
      <div className={`relative z-10 text-center max-w-4xl mx-auto px-4 transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        <div className="inline-flex items-center px-4 py-2 rounded-full bg-blue-50 text-[#0232ba] text-sm font-bold tracking-wide uppercase mb-8 shadow-sm border border-blue-100">
        ...
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-[#0232ba] leading-tight mb-8 drop-shadow-sm">
          Contabilidade <span className="text-[#0232ba]">Ágil e Segura</span>
        </h1>
        <p className="text-lg md:text-2xl text-gray-700 mb-10 leading-relaxed max-w-2xl mx-auto font-light">
          Exclusiva para SG, Niterói, Maricá e Itaboraí. <br className="hidden md:block"/>
...
const Benefits = () => {
  ...
  return (
    <div id="benefits" ref={ref} className="py-24 bg-white px-4">
      <div className={`text-center mb-16 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <h2 className="text-3xl md:text-5xl font-bold text-[#0232ba] tracking-tight">Por que nos escolher?</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {BENEFITS.map((benefit, index) => (
          <div 
            key={index} 
            className={`p-8 bg-blue-50/50 rounded-2xl hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-500 border border-blue-50 group hover:-translate-y-2
              ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}
            `}
            style={{ transitionDelay: `${index * 100 + 200}ms` }}
          >
            <div className="flex flex-col items-center text-center gap-4">
              <div className="p-4 bg-white rounded-xl shadow-sm group-hover:scale-110 transition-transform duration-300 ring-1 ring-blue-100">
                {benefit.icon}
              </div>
              <p className="text-lg font-medium text-[#0232ba] leading-snug">{benefit.text}</p>
            </div>
          </div>
        ))}
      </div>
...
const ServicesList = () => {
  ...
  return (
    <div id="services" ref={ref} className="py-24 bg-[#0232ba] text-white relative overflow-hidden px-4">
      {/* Decorative Blobs - White/Blue Overlay */}
      ...
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-blue-400 rounded-full mix-blend-overlay filter blur-[100px] opacity-20 animate-blob animation-delay-2000 pointer-events-none"></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10 w-full max-w-7xl mx-auto">
        <div className={`order-2 lg:order-1 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-20'}`}>
          <h2 className="text-3xl md:text-5xl font-bold mb-8 leading-tight text-white">Serviços Completos <br/> <span className="text-blue-200">Para sua Empresa</span></h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
            {SERVICES.map((service, index) => (
              ...
              >
                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-white/10 border border-white/20 group-hover:bg-white/20 flex items-center justify-center transition-colors">
                  <Check className="w-4 h-4 text-white" />
                </div>
                <span className="text-lg text-blue-100 group-hover:text-white transition-colors">{service.title}</span>
              </div>
            ))}
          </div>
        </div>
        
        <div className={`order-1 lg:order-2 flex flex-col gap-6 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-20'}`}>
           <div 
             className="bg-white/10 backdrop-blur-md p-8 rounded-3xl border border-white/10 hover:bg-white/15 transition-all duration-300 cursor-pointer shadow-xl group" 
           ...
           >
              <div className="flex justify-between items-start mb-4">
                ...
                <ArrowRight className="text-blue-200 group-hover:text-white group-hover:translate-x-2 transition-all" />
              </div>
              <h4 className="text-2xl font-bold text-white mb-2">Abertura de Empresa</h4>
              <p className="text-blue-100 text-sm leading-relaxed">Abra sua empresa com rapidez, segurança e orientação completa. Evite erros e multas.</p>
           </div>
           ...
           >
              ...
              <h4 className="text-2xl font-bold text-white mb-2">Regularização de CNPJ</h4>
              <p className="text-blue-100 text-sm leading-relaxed">Coloque sua situação fiscal em dia e volte a emitir notas sem bloqueios imediatamente.</p>
           </div>
        </div>
      </div>
    </div>
...
const Region = () => {
  ...
  return (
    <div id="region" ref={ref} className="py-24 bg-white px-4">
      <div className={`text-center w-full max-w-4xl mx-auto transition-all duration-1000 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
        <div className="inline-block p-4 rounded-full bg-blue-50 mb-8 animate-bounce">
            <MapPin className="w-8 h-8 text-[#0232ba]" />
        </div>
        <h2 className="text-4xl md:text-5xl font-bold text-[#0232ba] mb-6">Região de Atendimento</h2>
        <p className="text-xl text-gray-600 mb-16">
          Atendemos <strong className="text-[#0232ba]">exclusivamente</strong> estas cidades para garantir um serviço rápido e pessoal.
        </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {CITIES.map((city, index) => (
            <div 
                key={index} 
                className={`bg-white aspect-square p-6 rounded-3xl shadow-lg shadow-blue-900/5 border border-blue-50 flex flex-col items-center justify-center hover:scale-110 transition-all duration-500 group`}
                style={{ 
                    opacity: isVisible ? 1 : 0, 
            ...
            >
              <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-[#0232ba] transition-colors duration-300">
                <MapPin className="text-[#0232ba] w-8 h-8 group-hover:text-white transition-colors duration-300" />
              </div>
              <span className="font-bold text-[#0232ba] text-xl">{city}</span>
            </div>
          ))}
        </div>
      </div>
...
const SocialProof = () => {
  ...
  return (
    <div id="testimonials" ref={ref} className="py-24 bg-blue-50/30 px-4">
      <div className="w-full max-w-7xl mx-auto">
        <h2 className={`text-3xl md:text-5xl font-bold text-center text-[#0232ba] mb-16 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>Clientes Satisfeitos</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {TESTIMONIALS.map((t, i) => (
            <div 
                key={i} 
                className={`bg-white p-10 rounded-[2rem] relative hover:shadow-xl transition-all duration-700 hover:-translate-y-2 border border-blue-50`}
                style={{
                    opacity: isVisible ? 1 : 0,
            ...
            >
              <div className="absolute top-10 right-10 opacity-10">
                <Star size={64} className="text-[#0232ba]" />
              </div>
              <div className="flex gap-1 mb-6">
                {[...Array(t.rating)].map((_, r) => (
                  <Star key={r} className="w-6 h-6 text-yellow-400 fill-yellow-400" />
                ))}
              </div>
              <p className="text-gray-700 italic text-xl font-medium leading-relaxed mb-6">"{t.text}"</p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-[#0232ba] text-white rounded-full flex items-center justify-center text-sm font-bold">
                  {String.fromCharCode(65 + i)}
                </div>
                <div>
                  <div className="font-bold text-[#0232ba]">Cliente Verificado</div>
                  <div className="text-xs text-gray-500 uppercase tracking-wider">Google Review</div>
                </div>
              </div>
            </div>
          ))}
    ...
    <div ref={ref} className="py-24 bg-[#0232ba] text-white px-4">
      <div className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
        ...
        <div className={`space-y-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-20'}`}>
          ...
          <p className="text-blue-100 mb-8">Tudo o que você precisa saber antes de contratar.</p>
        ...
        <div className={`flex flex-col justify-between h-full space-y-12 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-20'}`}>
           ...
           <div className="text-center lg:text-right border-t border-white/20 pt-8">
              <div className="flex items-center justify-center lg:justify-end gap-2 mb-4">
                <div className="w-8 h-8 bg-white rounded flex items-center justify-center text-[#0232ba] font-bold">2A</div>
                <span className="text-white font-semibold text-lg">Contabilidade</span>
              </div>
              <p className="text-blue-200 text-sm">
                © {new Date().getFullYear()} Todos os direitos reservados. <br/>
                São Gonçalo, Niterói, Maricá e Itaboraí.
              </p>
           </div>
        </div>
